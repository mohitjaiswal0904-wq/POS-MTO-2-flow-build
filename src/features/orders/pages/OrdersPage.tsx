      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <header className="border-b border-slate-200 bg-white px-6 py-6">
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-[30px] font-semibold tracking-tight text-slate-900">Orders</h1>
              <p className="mt-1 text-base text-slate-500">Track and manage customer orders</p>
            </div>
            <button
              type="button"
              ...
              <tbody>
                {filtered.map((row) => (
                  <tr key={row.serialNo} className="border-b border-slate-100 last:border-0">
                    ...
                    <td className="px-4 py-4 font-medium text-slate-800">{row.orderId}</td>
